...
def my_func():
    ...
    if len(Date_start_entry.get()) < 10 and len(Date_end_entry.get()) < 10:
        messagebox.showwarning('Invalid Dates', 'Please enter date in YYYY-MM-DD format')
        start()
        return
...
def my_func2():
    ...
    company_label = Label(root, text="Company you want to analyze stock of -> ")
    company_label.grid(row=6, column=0, pady = 2,sticky = 'w')
    Radiobutton(root, text="1. Netflix", variable=z, value=1, command=lambda:miner(ds, de, c, z.get())).grid(row=7,column=0,sticky = 'w')
    Radiobutton(root, text="2. Facebook", variable=z, value=2, command=lambda: miner(ds, de, c, z.get())).grid(row=8,column=0,sticky = 'w')
    Radiobutton(root, text="3. Apple", variable=z, value=3, command=lambda:  miner(ds, de, c, z.get())).grid(row=9, column=0,sticky = 'w')
    Radiobutton(root, text="4. Amazon", variable=z, value=4, command=lambda: miner(ds, de, c, z.get())).grid(row=10, column=0,sticky = 'w')
    Radiobutton(root, text="5. Google", variable=z, value=5, command=lambda: miner(ds, de, c, z.get())).grid(row=11,column=0,sticky = 'w')
    Radiobutton(root, text="6. Bitcoin", variable=z, value=6, command=lambda: miner(ds, de, c, z.get())).grid(row=12,column=0,sticky = 'w')
    Radiobutton(root, text="7. Comparison of all stocks in that period", variable=z, value=7,command=lambda: miner(ds, de, c, z.get())).grid(row=12, column=0,sticky = 'w')
